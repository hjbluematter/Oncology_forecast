    ...
    <div>
      ...
      >
        <div>
          ...
          <p className="text-slate-600 text-xs">{metric.description} · unit: <span className="text-slate-400">{metric.unit}</span></p>
        </div>
    ...
    <div>
      ...
      >
        <div>
          ...
          <p className="text-slate-600 text-xs">Probability of technical & regulatory success · single value per combination (0–100%)</p>
        </div>
        <svg className={`w-4 h-4 text-slate-500 transition-transform ${open?"rotate-180":""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5"/>
        </svg>
            ...
            <table className="w-full border-collapse text-xs" style={{minWidth:360}}>
              <thead>
                <tr className="bg-slate-800/70 border-b border-slate-800">
                  ...
                  <th className="bg-slate-800/70 border-l border-slate-800 px-4 py-2 text-center font-medium text-slate-500" style={{minWidth:140}}>PTRS (%)</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={c.key} className={`border-b border-slate-800 ${dep?"bg-slate-900/40":"bg-slate-950"} ${c.isFirstInGeo&&c.geoIdx>0?"border-t-2 border-t-slate-700":""}`}>
                      ...
                      <td className="px-3 py-1.5 border-l border-slate-800">
                        {dep ? (
                          <div className="flex items-center gap-1.5">
                            <span className="w-28 px-2.5 py-1.5 text-slate-600 text-xs text-right tabular-nums">{displayVal || "—"}</span>
